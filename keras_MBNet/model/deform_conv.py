...
def tf_batch_map_coordinates(input, coords, order=1):
    ...
    coords_0 = tf.clip_by_value(coords[..., 0], 0, tf.cast(input_size_h, 'float32') - 1)
    coords_1 =tf.clip_by_value(coords[..., 1], 0, tf.cast(input_size_w, 'float32') - 1)
    coords =tf.stack([coords_0,coords_1],axis=-1)
    coords_lt = tf.cast(tf.floor(coords), 'int32')
    coords_rb = tf.cast(tf.ceil(coords), 'int32')
    coords_lb = tf.stack([coords_lt[..., 0], coords_rb[..., 1]], axis=-1)
    coords_rt = tf.stack([coords_rb[..., 0], coords_lt[..., 1]], axis=-1)
...
def tf_batch_map_offsets(input, offsets, order=1):
    ...
    grid = tf.meshgrid(
        tf.range(input_size_h), tf.range(input_size_w), indexing='ij'
    )
