...
vVal=0;
if any( ismember( vType, {'none'} ) ),        vVal=vVal+1;  end%通过vVal最后的数字判断是那几个的组合
...
if( format==0 )
  ...
  fId=fopen(fName);% 使用textscan必须先打开fopen opens the file filename for read access, and returns an integer file identifier
  if(fId==-1), error(['unable to open file: ' fName]); end; v=0;%若无法打开文件
  try v=textscan(fId,'%% bbGt version=%d'); v=v{1}; catch, end %#ok<CTCH> v=3
  if(isempty(v)), v=0; end
  % read in annotation (m is number of fields for given version v)
  if(all(v~=[0 1 2 3])), error('Unknown version %i.',v); end%若不存在这个版本
  frmt='%s %d %d %d %d %d %d %d %d %d %d %d';%person 416 216 24 45 1 0 0 0 0 0 0  注！！！ 第六个数1：代表有无遮挡  对于groundtruth，我们只需要使用前6项即可
  ms=[10 10 11 12]; m=ms(v+1); frmt=frmt(1:2+(m-1)*3);%12 v是指version m=12
  in=textscan(fId,frmt); for i=2:m, in{i}=double(in{i}); end; fclose(fId);%textscan可以理解成按frmt的格式在文本中进行获取 注：matlab数组下标从1开始
  % create objs struct from read in fields                                
  n=length(in{1}); objs=create(n);%in 是cell类型，n代表一幅图片中行人个数
  for i=1:n, objs(i).lbl=in{1}{i}; objs(i).occ=in{6}(i); end%lbl行人种类 + 行人是否有遮挡
  bb=[in{2} in{3} in{4} in{5}]; bbv=[in{7} in{8} in{9} in{10}];
  for i=1:n, objs(i).bb=bb(i,:); objs(i).bbv=bbv(i,:); end
  if(m>=11), for i=1:n, objs(i).ign=in{11}(i); end; end%frmt 数据格式  bb（2 3 4 5）occ（6） bbv（7 8 9 10）ign（11）ang（12）
  if(m>=12), for i=1:n, objs(i).ang=in{12}(i); end; end
elseif( format==1 )
  % load objs stored in PASCAL VOC format  PASCAL VOC格式的标签
...
elseif( format==2 )%ImageNet dev code
...
%2019.2.17暂时的理解是如果得到的框是有一定角度的，那么就获得摆正之后（变大）的边界框
...
if(nargin<3), pLoad={}; end%getFiles ->Get all corresponding files in given directories.
...
% load ground truth
persistent keyPrv gtPrv; key={gtDir,pLoad}; n=length(gtFs);%n总的标签数
...
if(iscell(dtFs)), dt0=cell(1,n);
  for i=1:n, dt1=load(dtFs{i},'-ascii');%forces load to treat the file as an ASCII file, regardless of the extension.
